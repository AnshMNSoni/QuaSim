...
export default function CircuitBuilder({ circuit, onAddGate, onRemoveGate }: CircuitBuilderProps) {
  ...
  const GATE_SIZE = 40 // Fixed gate size in pixels
  const GATE_GAP = 8 // Gap between gates in pixels
  const STEP_WIDTH = GATE_SIZE + GATE_GAP // Total width per step
  ...
  return (
    <div className="w-full">
      ...
      <div className="overflow-x-auto overflow-y-visible -mx-4 px-4 pb-4">
        <div style={{ minWidth: `${80 + maxSteps * STEP_WIDTH + 20}px` }}>
          {/* Step numbers */}
          <div className="flex mb-4 items-center">
            <div className="w-20 flex-shrink-0"></div>
            <div className="flex gap-2">
              {Array.from({ length: maxSteps }).map((_, step) => (
                <div key={step} style={{ width: `${GATE_SIZE}px` }} className="text-center text-xs text-gray-500 dark:text-gray-400">
                  {step + 1}
                </div>
              ))}
            </div>
          ...
          {Array.from({ length: circuit.numQubits }).map((_, qubit) => (
            <div key={qubit} className="flex mb-4 items-center relative">
              {/* Qubit label */}
              <div
                className={`w-20 flex-shrink-0 text-right pr-4 font-mono ${
              ...
              <div className="flex gap-2 items-center relative">
                {/* Qubit line behind gates */}
                <div
                  className="absolute h-px bg-gray-300 dark:bg-gray-700"
                  style={{
                    left: `${GATE_SIZE / 2}px`,
                    right: `${GATE_SIZE / 2}px`,
                    top: "50%",
                    zIndex: 0,
                  }}
                />
